...
A Case Workspace tool (`components/case-details/PointsCalculator.tsx`, opened from `CaseDetails.tsx`'s tool catalog) that scores the GSM points test for subclasses 189/190/491 and maps each claim to the Case Files that evidence it.

- **Rule table**: `lib/pointsTest.ts` authors the points test as structured data with a per-criterion `source` citation, following the `lib/documentTypes.ts` convention of writing judgment calls next to the entry they affect. **The values were authored from general knowledge, with no live lookup of the DoHA tables or the Federal Register of Legislation — they need periodic human/legal review**, and `POINTS_TEST_DISCLAIMER` (rendered in the tool and in both exports) must keep saying so.
- **Claimed / Proven / Outstanding**: derived, never stored. `calculatePoints()` treats a criterion as *proven* when at least one linked Case File still exists, so deleting a file immediately makes the claim outstanding again. Cap groups (overseas + Australian skilled employment, max 20) are applied to claimed and proven alike.
- **Evidence mapping**: `suggestEvidenceForCriterion()` matches a criterion's `evidenceDocumentTypeCodes` against `Document.documentTypeCode` — the same shared vocabulary as checklist auto-link — but deliberately ignores the Document Type's `autoLink` flag and never links anything by itself; a points claim is an assertion the agent signs off on. No new Document Types were needed: `lib/documentTypes.ts`'s "State Nomination & Points" category plus ENGTST/DEGCRT/REFLTR/PAYSLP already cover points evidence.
- **Persistence**: `CasePointsClaim` (one record per case) through `repos.pointsClaims` — `points-claims/{caseId}.json` locally, and the `points_claims` table (entries as jsonb, unique on `(user_id, case_id)`, RLS-scoped to `auth.uid()`) in cloud mode. Migration at `supabase/migrations/20260830000000_create_points_claims.sql`, not applied automatically.
- **Exports** (`lib/pointsExport.ts`, built on the pdf-lib dependency `lib/pdfBundle.ts` already uses): a full PDF breakdown for the client file, and a shorter cover-letter points summary for an EOI/nomination lodgement. Both are framed as drafts for human review, the same way the Reference Letter Generator's output is.
