...
Either way, `App.tsx`'s `handleTasksConfirmed()` writes a `tasks_planned` `ActivityEvent` whose
wording matches which flow actually ran — `lib/planActivity.ts`'s `describePlanSource()` classifies
the new tasks by `generatedByAi`/`stepKey` (`'template'` | `'ai'` | `'mixed'`, e.g. after gap tasks
or AI-suggested additions are mixed in) and `buildTaskPlanSummary()` renders it as "Created a
12-task plan from the *X* template" or "Generated a 12-task plan with AI" — never the reverse. This
also backs `pages/Dashboard.tsx`'s "Recent activity" panel (renamed from "Agent activity", since not
every entry is AI work): it renders these events directly, falling back to the same
`lib/planActivity.ts` wording (built from the case's tasks) for old cases created before this event
type existed.
...
- **No more fake team**: `lib/seedData.ts`'s `seedDefaultTeam()` and `App.tsx`'s first-launch
  ...
  owner pickers therefore only ever list real people. A folder linked before this cutover can still
  have the old fictional records (`tm-eliza-chen`/`tm-marcus-okafor`/`tm-priya-singh`, from the
  earlier `seedDefaultTeam()`) sitting in `team-members/`, since local mode has no migration path —
  `lib/sampleTeamMembers.ts`'s `isSampleTeamMember()`/`findSampleTeamMembers()` (id-based, not
  name-based, so a real person can't collide) back a dismissible notice on `pages/TeamMembers.tsx`'s
  local-mode view offering to remove them (unassigning, never deleting, their old cases/tasks via
  the same `onDeleteMember` path a normal removal uses). That local-mode view also labels its "add
  person" action honestly (**Add person**, not **Invite** — Local Storage is single-user, so nobody
  can actually sign in) and calls its cosmetic Partner/Lawyer/Assistant column **Title** rather than
  **Role**, to avoid reading like the Cloud Storage access roles (Owner/Admin/Member) above.
