...
7. "Open Case" button (same label on both the hero card and each pathway card; a "Possible match" badge appears next to it for `possibly_qualifies` verdicts) on qualified visas drafts the AI task plan first, then resolves the client — preferring the client the page was opened for (`?clientId=`) over name/DOB matching, and requiring a non-empty name in step 1 of the wizard — then creates the case directly and navigates to its detail page (button shows staged progress: plan → client → finalizing; Start Over is disabled while creating). If case creation fails after a new client was created, that client is rolled back. If the AI plan fails, the case is still created (with a toast saying tasks can be generated from the case page) rather than blocking case creation. If the user has navigated away from Visa Advisor by the time creation finishes, a toast with a "View case" action is shown instead of navigating out from under them.
