...
- **Backend:** Vercel serverless functions in root `api/` directory (not `src/api/`):
  ...
  - `/api/scan-lmt-evidence-gemini.ts` — extracts job-ad platform/start/close dates from LMT evidence (image or PDF) using Gemini Vision
...
Labour Market Testing evidence for employer-sponsored nominations is modelled as structured records, not just a checklist line.

- **Data**: `LmtAdRecord` in `types.ts` (`platform`, `startDate`, `endDate`, optional `documentId` pointing at the `LMTEVD`-tagged evidence `Document`), persisted through `repos.lmtAds` in both modes (`lmt-ads/{caseId}/{id}.json` locally, mirroring `case-notes/`; the `lmt_ad_records` table in cloud, migration `supabase/migrations/20260830000000_create_lmt_ad_records.sql`).
- **Rules** live in `lib/lmt.ts`, never inline in a component: `LMT_SUBCLASSES` (`482`/`494`/`186` — the feature is gated by subclass membership, not an `=== '482'` check), `computeLmtWindow()` (window expiry = the **latest** ad's `endDate` + `LMT_WINDOW_MONTHS`), and `inferNominationLodged()`.
- **Nomination-lodged signal** is inferred from existing case state rather than a new entity: a Document Checklist item using the `NOMAPP` Document Type (or whose label mentions "nomination") at linked/verified/waived, **or** a completed Task whose title reads as nomination lodgement. `CaseDetails.tsx`'s "Mark nomination as lodged" writes through that same checklist item.
- **OCR**: `api/scan-lmt-evidence-gemini.ts` (images + PDF, same inline-base64 shape as `scan-passport-gemini.ts`) → `services/lmtOcrService.ts` → `components/LmtAdScanner.tsx`, which always shows extracted values in an editable confirm form. Dates are only accepted from the model when they match `^\d{4}-\d{2}-\d{2}$`.
- **Surfaces** (all in `CaseDetails.tsx`): an `LmtExpiryBanner` above the case grid, a rail alert, a persisted `Notification` with a deterministic id (`lmtNotificationId()`) so a window notifies once, and the `lmt-evidence` Workspace tool tab rendering `components/case-details/LmtEvidencePanel.tsx`. The banner appears only when the window is within 30 days or lapsed **and** the nomination isn't lodged; when lapsed, its primary CTA opens the add-advertisement flow for a fresh campaign.
